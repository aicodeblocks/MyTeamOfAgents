...
- `correlation_id`: required correlation identifier linking the recommendation to the originating request, thread, or workflow context
...
2. **Review outcome**: latest review result from Olivia or another authorized reviewer
   ...
   - examples: `pending`, `approved`, `rejected`, `needs_revision`
   - conditional approval must not introduce a separate `review_outcome` value and is represented only in Story 4 as `approved` plus structured `conditions`
...
2. `correlation_id` must remain aligned to the originating request, thread, or workflow context for the recommendation lineage.
3. `recommendation_version` must increment for each revised recommendation version.
4. Each review must reference an exact `recommendation_id` plus `recommendation_version`.
5. If a version replaces a prior version, supersession must be explicitly represented.
6. `recommendation_status`, `review_outcome`, and `execution_state` must be stored separately and interpreted independently.
7. A recommendation may be:
   - `active` while `review_outcome = pending`
   - `active` while `review_outcome = approved`
   - `superseded` even if `execution_state = completed`
8. `recommended_action` must be structured enough for downstream systems to interpret without relying on free text alone.
9. `source_references` must be sufficient for auditability.
10. `rationale_summary` must be human-readable and concise, but not replace structured provenance.
...
```json
{
  ...
  "correlation_id": "corr_01JQZ8F2C9H4K6M8N1P3R5T7V9",
